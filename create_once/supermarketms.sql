...
CREATE TRIGGER `before_update_user` BEFORE UPDATE ON `users` FOR EACH ROW BEGIN
    ...
    VALUES (OLD.userid, OLD.name, OLD.userName, OLD.password,OLD.Type,OLD.isActive,OLD.salary,OLD.transactionCount,NOW());
    -- Note: OLD is replaced with NEW in tables.php
END
$$
DELIMITER ;
DELIMITER $$
